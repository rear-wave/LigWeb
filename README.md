...
每天 22:00，主模型任务先扫描 `correct_data\IC`，按波形内容哈希与
`train_data\IC` 全量去重。新片段写入训练集并回读校验成功后，才删除对应的
纠错集源 `.lig`；训练集中已有的重复片段不会再次写入，确认已存在后同样清理
纠错集源文件。迁移失败时保留尚未处理的源文件并终止当晚训练，避免数据丢失。
网页运行期间不再周期复制 IC 数据。
...
- 纠错模型与主模型哈希绑定；主模型更新后会自动训练兼容的新纠错代次，
  不会继续加载旧主模型的纠错权重。
- 主模型：每天 22:00 先迁移并清空纠错集 IC，再使用 `conda` 的
  `ligclassify` 环境训练。
