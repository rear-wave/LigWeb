...
每天 22:00，主模型任务先将 `correct_data\IC` 按波形内容哈希去重，写入
临时目录并回读校验。校验成功后，使用这个完整副本整体替换
`train_data\IC`；纠错集源文件始终保留、不修改。复制或校验失败时继续保留
原训练集并终止当晚训练。网页运行期间不执行周期复制。
...
- 纠错模型与主模型哈希绑定；主模型更新后会自动训练兼容的新纠错代次，
  不会继续加载旧主模型的纠错权重。
- 主模型：每天 22:00 先用纠错集完整重建训练集 IC，再使用 `conda` 的
  `ligclassify` 环境训练。
