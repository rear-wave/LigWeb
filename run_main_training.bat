...
set "FEEDBACK_DIR=%CORRECTION_DATA%\.ligedit"
...
set "TRAINING_LOG=%FEEDBACK_DIR%\main_model\scheduled-training.log"
...
if not exist "%FEEDBACK_DIR%\main_model" mkdir "%FEEDBACK_DIR%\main_model"
echo [%date% %time%] nightly main-model training started>>"%TRAINING_LOG%"
echo Python: %LIGCLASSIFY_PYTHON%>>"%TRAINING_LOG%"
"%LIGCLASSIFY_PYTHON%" -m tools.train_main_model --train-data "%TRAIN_DATA%" --correction-data "%CORRECTION_DATA%" --feedback-dir "%FEEDBACK_DIR%" --ligclassify-root "%LIGCLASSIFY_ROOT%" >>"%TRAINING_LOG%" 2>&1
