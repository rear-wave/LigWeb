...
docker run -d --name ligweb --restart unless-stopped -p %LIGWEB_PORT%:8000 -v "%TRAIN_DATA%:/data/train" -v "%CORRECTION_DATA%:/data/correction" -e LIGWEB_BASE_MODEL_PATH=/data/correction/.ligedit/main_model/current.onnx -e LIGWEB_BASE_MODEL_METADATA_PATH=/data/correction/.ligedit/main_model/current.json -e LIGWEB_AUTO_CORRECTION_TRAINING=1 -e LIGWEB_AUTO_IC_SYNC=1 ligweb
if errorlevel 1 exit /b 1
